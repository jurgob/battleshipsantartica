...
public class BSView extends TableLayout implements BSUser {
	...
	private class UserStrategy implements BSStrategy, OnTouchListener {
		...
		public synchronized boolean onTouch(View v, MotionEvent event) {
			...
			if(fieldView.getPlayerToShow() != ME){
				...
				int sx = (int) (x / w);
				int sy = (int) (y / h);
				if(bs.getField(fieldView.getPlayerToShow())[sx][sy]!=State.SEA_HITTED && bs.getField(playerToShow)[sx][sy]!=State.SHIP_HITTED ){
					System.out.println("SELEZIONE VALIDA");
					
					fieldView.selectTile(event);
					notify();
				}else{
					System.out.println("SELEZIONE NON VALIDA: " +sx+" "+sy);
	...
	public void update(Observable observable, Object data) {
		...
		if(bs.over()){
			...
			if(bs.conflictOn()) messageLabel.setText("Conflict On!");
