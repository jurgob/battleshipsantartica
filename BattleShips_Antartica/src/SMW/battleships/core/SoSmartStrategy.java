...
public class SoSmartStrategy implements BSStrategy{
	...
	int[] suggX={-1,0,1,0};
	int[] suggY={0,1,0,-1};
	boolean[] directionTryed={false, false, false,false};
	boolean[] directionWithHit={false, false, false,false};
	int tryDirection=0;
	Move firstHitted=null;

	public SoSmartStrategy() {
	...
	private boolean [] getFalseTryArray(){
	...
	try{	
		if(lastHit && state==0 ){
			...
			directionTryed =getFalseTryArray();
			directionWithHit=getFalseTryArray();
...
//				state++;
		...
		if(m==null &&  !lastHit && state == 1 && !directionTryed[2] ){
		...
		if( m==null&& !lastHit && state == 2 && !directionTryed[0]){
			System.out.println("-CAMBIO DA VERTICALE A ORIZZONTALE!!!!");
			if(firstHitted != null){
					System.out.println("--FIRST HIT PRESENT!!!!");
		...
		if(m==null &&  !lastHit && state == 3 && !directionWithHit[0] && !directionWithHit[2] ){
